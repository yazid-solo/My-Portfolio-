...
function renderFeaturedProjects() {
  ...
  const AUTO_ROT_SPEED = 0.2; // kecepatan mutar 3D dinormalkan kembali
    ...
    <div class="glass-card tilt-card reveal reveal-delay-${i+1} training-card-item" style="display: ${i < displayCount ? 'block' : 'none'}; padding:0; position:relative; overflow:hidden; border:1px solid rgba(0,212,255,0.2); box-shadow:0 8px 32px rgba(0,0,0,0.3); border-radius:18px;">
      ...
      <div class="training-card-body" style="padding:1.75rem; position:relative; z-index:2; display:grid; grid-template-columns: 1fr auto; gap:1.5rem; align-items:center;">
        ...
        <div>
          <div style="display:flex; flex-wrap:wrap; align-items:center; gap:0.65rem; margin-bottom:0.75rem;">
            <span style="font-size:0.75rem; font-family:var(--font-mono); font-weight:700; color:var(--bg); background:var(--accent); padding:3px 10px; border-radius:6px; letter-spacing:0.02em; display:inline-flex; align-items:center;">
               ${window.t({id: "PELATIHAN", en: "TRAINING"})}
            </span>
        ...
        <div class="training-card-btns" style="display:flex; flex-direction:column; gap:0.65rem;">
          ${t.image ? `<button onclick="openImageModal('${t.image}')" class="btn-ghost" style="padding:8px 14px; font-size:0.75rem;"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M15 3h6v6M9 21H3v-6M21 3l-7 7M3 21l7-7"/></svg> ${window.t({id: "Lihat Sertifikat", en: "View Certificate"})}</button>` : ''}
  ...
  if (isTrainingExpanded) {
    ...
    btnContainer.innerHTML = `
      <button onclick="toggleTraining()" class="btn-ghost" style="padding:10px 24px; font-weight:600; border-radius:99px; display:inline-flex; align-items:center; gap:8px;">
        ...
        ${window.t({id: "Lihat Semua", en: "View All"})} (${PROFILE.training.length})
      </button>
    `;
  }
}
             ...
             <div style="flex: 1 1 0%; padding:2.5rem 2rem; display:flex; flex-direction:column; justify-content:center; position:relative;">
                ...
                <div style="position:relative; z-index:1;">
                  ...
                  <div style="display:flex; align-items:center; flex-wrap:wrap; gap:1rem;">
                    ${edu.current ? '<span style="display:inline-flex;align-items:center;gap:6px;font-size:0.75rem;color:#10b981;background:rgba(16,185,129,0.1);padding:6px 14px;border-radius:99px;font-weight:700;border:1px solid rgba(16,185,129,0.3);box-shadow:0 0 12px rgba(16,185,129,0.2);"><span style="width:8px;height:8px;border-radius:50%;background:#10b981;box-shadow:0 0 10px #10b981; animation:pulse-dot 2s infinite;"></span> ' + (window.I18N && window.I18N.dict ? window.t(window.I18N.dict.lbl_in_progress) : 'Sedang Ditempuh') + '</span>' : '<span style="display:inline-flex;align-items:center;gap:6px;font-size:0.75rem;color:var(--text-dim);font-weight:600;padding:6px 14px;background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.1);border-radius:99px;"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3"><polyline points="20 6 9 17 4 12"></polyline></svg> ' + (window.I18N && window.I18N.dict ? window.t(window.I18N.dict.lbl_graduated) : 'Lulus / Selesai') + '</span>'}
